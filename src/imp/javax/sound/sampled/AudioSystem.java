...
import imp.javax.sound.sampled.spi.AudioFileReader;
import imp.javax.sound.sampled.spi.AudioFileWriter;
import imp.javax.sound.sampled.spi.FormatConversionProvider;
import imp.javax.sound.sampled.spi.MixerProvider;
 ...
 * name of a concrete {@link imp.javax.sound.sampled.spi.MixerProvider
...
public class AudioSystem {
    ...
    public static Mixer.Info[] getMixerInfo() {
        ...
        Mixer.Info[] allInfos = (Mixer.Info[]) infos
                .toArray(new Mixer.Info[infos.size()]);
        return allInfos;
    }
    ...
    public static Mixer getMixer(Mixer.Info info) {
        ...
        for (int i = 0; i < providers.size(); i++) {
            ...
            try {
                return ((MixerProvider) providers.get(i))
                        .getMixer(info);
        ...
        if (info == null) {
            for (int i = 0; i < providers.size(); i++) {
                try {
                    MixerProvider provider = (MixerProvider) providers
                            .get(i);
    ...
    public static AudioInputStream getAudioInputStream(
        ...
        for (int i = 0; i < codecs.size(); i++) {
            ...
            if (codec.isConversionSupported(targetFormat, sourceStream
                    .getFormat())) {
                return codec.getAudioInputStream(targetFormat,
                        sourceStream);
            }
        }
    ...
    public static AudioFileFormat.Type[] getAudioFileTypes() {
        ...
        for (int i = 0; i < providers.size(); i++) {
            ...
            AudioFileFormat.Type[] fileTypes = writer
                    .getAudioFileTypes();
     ...
     * @return an array of
     * {@link imp.javax.sound.sampled.spi.FormatConversionProvider
     ...
     * @return a List of
     * {@link imp.javax.sound.sampled.spi.AudioFileReader
     * AudioFileReader}
    ...
    private static Mixer getDefaultMixer(List providers, Line.Info info) {
        ...
        String providerClassName = JDK13Services
                .getDefaultProviderClassName(lineClass);
        String instanceName = JDK13Services
                .getDefaultInstanceName(lineClass);
        Mixer mixer;

        if (providerClassName != null) {
            MixerProvider defaultProvider = getNamedProvider(
                    providerClassName, providers);
            if (defaultProvider != null) {
                if (instanceName != null) {
                    mixer = getNamedMixer(instanceName,
                            defaultProvider, info);
                    if (mixer != null) {
                        return mixer;
                    }
                } else {
                    mixer = getFirstMixer(defaultProvider, info, false /* mixing not required*/);
                    if (mixer != null) {
                        return mixer;
                    }
                }
    ...
    found.
     */
    private static MixerProvider getNamedProvider(
            String providerClassName, List providers) {
        for (int i = 0; i < providers.size(); i++) {
            MixerProvider provider = (MixerProvider) providers.get(i);
    ...
    private static Mixer getNamedMixer(String mixerName,
            MixerProvider provider, Line.Info info) {
        Mixer.Info[] infos = provider.getMixerInfo();
    ...
    private static Mixer getFirstMixer(MixerProvider provider,
            Line.Info info, boolean isMixingRequired) {
        Mixer.Info[] infos = provider.getMixerInfo();
        for (int j = 0; j < infos.length; j++) {
            Mixer mixer = provider.getMixer(infos[j]);
    ...
    private static boolean isAppropriateMixer(Mixer mixer,
            Line.Info lineInfo, boolean isMixingRequired) {
        if (!mixer.isLineSupported(lineInfo)) {
            return false;
        }
        Class lineClass = lineInfo.getLineClass();
        if (isMixingRequired
                && (SourceDataLine.class.isAssignableFrom(lineClass) || Clip.class
    ...
    private static List getMixerInfoList(List providers) {
        ...
        for (int i = 0; i < providers.size(); i++) {
            someInfos = ((MixerProvider) providers
                    .get(i)).getMixerInfo();
