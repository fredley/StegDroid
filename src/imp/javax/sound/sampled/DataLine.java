...
public interface DataLine extends Line {
    ...
    public static class Info extends Line.Info {
         ...
         * {@link imp.javax.sound.sampled.AudioSystem#NOT_SPECIFIED NOT_SPECIFIED}
        ...
        public AudioFormat[] getFormats() {
            ...
            System.arraycopy(formats, 0, returnedArray, 0,
                    formats.length);
		...
		public boolean matches(Line.Info info) {
            ...
            if ((getMaxBufferSize() >= 0)
                    && (dataLineInfo.getMaxBufferSize() >= 0)) {
                if (getMaxBufferSize() > dataLineInfo
                        .getMaxBufferSize()) {
                    return false;
                }
            }
            ...
            if ((getMinBufferSize() >= 0)
                    && (dataLineInfo.getMinBufferSize() >= 0)) {
                if (getMinBufferSize() < dataLineInfo
                        .getMinBufferSize()) {
                    return false;
                }
            }
            ...
            if (localFormats != null) {
                ...
                for (int i = 0; i < localFormats.length; i++) {
                    if (!(localFormats[i] == null)) {
                        if (!(dataLineInfo
                                .isFormatSupported(localFormats[i]))) {
                            return false;
                        }
                    }
                }
